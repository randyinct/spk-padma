...
class RankingController extends Controller
{
    public function perhitungan(Alternatif $alternatif)
    {
        ...
        for($i = 0; $i < $rowsK; $i++){
            ...
            // UNTUK DATA PERANKINGAN
            $altName = Alternatif::select('alternatif')->where('user_id', auth()->user()->id)->pluck('alternatif')->toArray();
            $vektorV = $hasilV;

            $baru = [];
                foreach($altName as $x => $xval) 
                {
                    foreach($hasilV as $y => $yval) 
                    {
                    if ($x == $y) 
                        {
                    $baru[] = [
            ...
            return view('dashboard.ranking.index', compact('altName', 'vektorV', 'rowsAlt','baru'));
        } else {
            return view('dashboard.ranking.index', compact( 'rowsAlt'));
        }
